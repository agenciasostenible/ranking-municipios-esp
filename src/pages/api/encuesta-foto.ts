/**
 * GET /api/encuesta-foto?id=NN
 * Sirve la foto de perfil de una respuesta de la encuesta (data URL base64 en D1).
 */
...
export const GET: APIRoute = async ({ url }) => {
  ...
  const row = await DB.prepare(
    `SELECT avatar_data, avatar_mime FROM encuesta WHERE id = ? LIMIT 1`
  ).bind(id).first();
